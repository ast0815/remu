...
parser = argparse.ArgumentParser(description="Modify the reconstructed events of a simulation.")
...
# Toy detectors
toy_detectors = []
for slope, offset, sigma,  max_eff in zip(eff_slopes, eff_offsets, smear_sigmas, max_effs):
    toy_detectors.append(experiment.Detector(eff_slope=slope, eff_offset=offset, smear_sigma=sigma, max_eff=max_eff))
...
for i, toy in enumerate(toy_detectors):
    weights.append(np.array(toy.efficiency(events) / nominal, dtype=[('weight_%i'%(i,), float)]))
